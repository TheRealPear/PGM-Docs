...
Once you are done with the initial setup, you can double-click `start.command` to let SportPaper create the necessary files for Minecraft and PGM.
You can now join the server from your Minecraft client at `localhost` or `127.0.0.1:25565`.
...
Please keep in mind that the following process may differ depending on your distribution. If necessary, consult your distribution's documentation or user manual.
Otherwise, we trust that you know what you are doing.

### Installing Java

We recommend using Java 21. For this guide, we will be using [OpenJDK](https://openjdk.org/). 
...
experiments:
  # Set to "true" if you plan on creating and testing Payload maps.
  payload: true
```
